...
# read in data
RI_1B2_raw_pub_prod_tbl <- read_excel(here::here("data", "raw", "labourproductivitylad1.xls"),
                             sheet = "B3",
                             skip = 4) ##|> 
  ##glimpse()
...
weca_pub_prod_data_clean <- weca_pub_prod_data |> 
  ...
  pivot_longer(pounds_2013:pounds_2023, names_to = "year", values_to = "gva_per_job") |> 
  mutate(year = str_remove(year,"pounds_"),
...
# read in data
RI_1B2_raw_pub_jobs_tbl <- read_excel(here::here("data", "raw", "labourproductivitylad1.xls"),
...
weca_pub_jobs_data_clean <- weca_pub_jobs_data |> 
  ...
  pivot_longer(jobs_2013:jobs_2023, names_to = "year", values_to = "prod_jobs") |> 
  mutate(year = str_remove(year,"jobs_"),
         Date = glue("{year}-01-01") |> 
           as.Date()) ##|> 
  ####glimpse()
...
RI_1B2_gva_per_job_tbl <- weca_pub_jobs_data_clean |> 
  inner_join(weca_pub_prod_data_clean, by = join_by("Date" == "Date", "lad_name" == "lad_name")) |> 
  mutate(gva = prod_jobs * gva_per_job) |> 
  group_by(Date) |> 
  summarise(weca_gva = sum(gva), weca_jobs = sum(prod_jobs), weca_prod = weca_gva / weca_jobs) |> 
...
# plots data
RI_1B2_plot <- ggplot(RI_1B2_gva_per_job_tbl, aes(x = period_start, y = value)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  labs(title = "Annual GVA per job, 2013-2023",
